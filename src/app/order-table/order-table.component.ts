...
export class OrderTableComponent implements OnInit 
{
  ...
  ngAfterViewInit() 
  {
    ...
    this.dataSource.sortingDataAccessor = (item: Order, property: string) => 
    {
      switch (property) 
      {
        case 'customer': return `${item.first_name} ${item.last_name}`.toLowerCase();
        case 'date': return this.parseCustomDate(item.data);
        default:
          // @ts-ignore
  ...
  onRowClick(event: MouseEvent, order: Order): void 
  {
    ...
    {
      if (order.status !== 'archive') this.editOrder(event, order);
    }
  }
